...
| REQ-018 | Multilingual Article System | Implement language-specific article content with automatic language detection and fallback | High | Done | @khaled | Articles support en.json, de.json, tr.json, ar.json files per folder. Component loads articles in user's selected language with English fallback. Build generates multilingual data structure. | scripts/generate-articles.js, config/content/inspiration/*/*.json, config/content/inspiration/*/*.html, components/Inspiration.tsx |
| REQ-017 | Complete Article-Language Separation | Ensure article content is completely independent from language JSON files | High | Done | @khaled | All 'featuredArticles' arrays removed from de.json, en.json, tr.json, ar.json. Articles only load from /config/content/inspiration/ folders with zero JSON dependencies | config/content/*.json, components/Inspiration.tsx |
