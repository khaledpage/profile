...
import { canSavePreferences, shouldShowCookieBanner, getCookieConsent, setCookieConsent, type CookieConsent } from '@/utils/cookies';
...
export default function SettingsPanel({ config }: Props) {
  ...
  const handleCookieConsent = (accept: boolean) => {
    ...
    setCookieConsentState(consentData);
  ...
  const themes = {
    default: { name: 'Default', background: '#ffffff', foreground: '#000000' },
    dark: { name: 'Dark', background: '#1a1a1a', foreground: '#ffffff' },
    blue: { name: 'Blue', background: '#1e3a8a', foreground: '#ffffff' }
  };
  const availableLanguages = config?.i18n?.languages || { en: 'English' };

  const tabs = [
    { id: 'general' as TabId, label: 'General', icon: '⚙️' },
    { id: 'themes' as TabId, label: 'Themes', icon: '🎨' },
    { id: 'privacy' as TabId, label: 'Privacy', icon: '🔒' }
  ];
                  ...
                  <div>
                    <label className="flex items-center justify-between">
                      ...
                      <span className="text-sm" style={{ 
                        color: cookieConsent?.analytics ? 'var(--success)' : 'var(--warning)' 
                      }}>
                        {cookieConsent?.analytics ? 'Granted' : 'Not granted'}
                      </span>
                    </label>
